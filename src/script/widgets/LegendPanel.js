 ...
 */
gxp.LegendPanel = Ext.extend(Ext.Panel, {
    ...
    addLegend: function(record, index) {
        if (this.filter(record) === true) {
            ...
            index = index || 0;
            var legend;
            if(layer.displayInLayerSwitcher && !record.get('hideInLegend') && layer instanceof OpenLayers.Layer.WMS ) {
                this.insert(index, {
                    xtype: 'gxp_wmslegend',
...
/** api: xtype = gxp_legendpanel */
Ext.reg('gxp_legendpanel', gxp.LegendPanel);
